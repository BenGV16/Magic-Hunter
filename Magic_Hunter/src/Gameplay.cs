...
public class GamePlay
{
    ...
    private double _mudBlindTimer = 0; 
    private Texture2D _mudOverlayTexture;
    private Vector2 _mudPosition; 
    ...
    public void Reset() { _entities.Clear(); _projectiles.Clear(); _enemyWaveQueue.Clear(); _waveNumber = 0; _playerHealth = _playerMaxHealth; _shieldDurability = _maxShieldDurability; _isShieldBroken = false; _difficultyMultiplier = 1.0f; _flashbangAlpha = 0f; _mudBlindTimer = 0; IsGameOver = false; StartNextWave(); }
    
    private void StartNextWave() 
    { 
        // CAMBIO: Curación al inicio de la oleada
        if (_waveNumber > 0) // Si no es la primera partida (reinicio)
        {
    ...
    public void Update(GameTime gameTime, Viewport viewport, MouseState mouseState)
    {
        ...
        if (_mudBlindTimer > 0) { _mudBlindTimer -= delta; }
        ...
        if (_waveInProgress)
        {
            ...
            { 
                ...
                { 
                    ...
                    { 
                        ...
                        // CAMBIO: Verificación en tiempo real para NO tener 2 sapos vivos simultáneamente
                        if (candidateType == 2) 
                        {
                            bool sapoExists = _entities.Any(e => e is Sapo);
                            if (sapoExists)
                            {
                                // Cambiar a otro tipo hasta que no sea sapo
                                do {
                                    candidateType = _random.Next(5);
            ...
            { 
                ...
                _waveBreakTimer = 3.0; 
                // Eliminada curación de aquí, movida a StartNextWave
            }
        }
        ...
        for (int i = _entities.Count - 1; i >= 0; i--)
        {
            ...
            if (entity is Lobo lobo && lobo.IsAttacking)
            {
            ...
            if (entity is Sapo sapo)
            {
                ...
                {
                    SpawnProjectile(sapo.Rect.Center.ToVector2(), viewport, _projectileTexture, false, false);
            ...
            else if (entity is Gusano gusano)
            {
                ...
                {
                    SpawnProjectile(gusano.Rect.Center.ToVector2(), viewport, _arenaProjectileTexture, true, false);
                    gusano.ReadyToShoot = false;
                }
            }
            else if (entity.Damage > 0 && !(entity is Hada) && !(entity is Lobo) && !(entity is Hada2) && !(entity is Gusano))
            {
                if (_random.NextDouble() < 0.005 * _difficultyMultiplier) SpawnProjectile(entity.Rect.Center.ToVector2(), viewport, _projectileTexture, false, false);
            }
        }
        ...
        if (mouseState.LeftButton == ButtonState.Pressed && _previousMouseState.LeftButton == ButtonState.Released && mouseState.RightButton == ButtonState.Released)
        {
            ...
            SpawnProjectile(startPos, viewport, _playerProjectileTexture, false, true, mousePos.ToVector2());
        }
        _previousMouseState = ms;
        _playerWand?.Update(gameTime);
    }

    private void SpawnProjectile(Vector2 startPos, Viewport viewport, Texture2D texture, bool isMud, bool isPlayer, Vector2? targetPos = null)
    {
    ...
    private void UpdateProjectiles(GameTime gameTime, Viewport viewport, bool isShielding, Point mousePos)
    {
        ...
        for (int i = _projectiles.Count - 1; i >= 0; i--)
        {
            ...
            if (p.ActivationDelay > 0)
            {
                ...
                if (!p.IsPlayerProjectile) p.Scale += 0.5f * delta; 
            ...
            if (p.IsPlayerProjectile)
            {
                ...
                else { p.StopDecayTimer -= delta; if (p.StopDecayTimer <= 0) { _projectiles.RemoveAt(i); continue; } }
            }
            else 
            {
                p.Scale += 0.5f * delta; 
            ...
            Rectangle pRect = new Rectangle((int)(p.Position.X - pW / 2), (int)(p.Position.Y - pW / 2), pW, pW);
            ...
            if (p.IsPlayerProjectile)
            {
                ...
                for (int j = _entities.Count - 1; j >= 0; j--)
                {
                    ...
                    {
                        if (_entities[j].TakeDamage(p.Damage)) { if (_entities[j] is Hada2) _flashbangAlpha = 1.5f; _entities.RemoveAt(j); }
                        hit = true; break; 
                    }
                }
            ...
            else 
            {
                if (p.Scale > 1.5f && !p.IsMudEffect) 
                {
                    ...
                    if (isShielding)
                    {
                        ...
                        {
                            blocked = true; _blockFeedbackTimer = 0.2; 
    ...
    public void Draw(SpriteBatch spriteBatch)
    {
        ...
        if (_mudBlindTimer > 0 && _mudOverlayTexture != null)
        {
            int size = 800;
            Rectangle mudRect = new Rectangle((int)_mudPosition.X - size/2, (int)_mudPosition.Y - size/2, size, size);
        ...
        if (_showingWaveMessage && _font != null) { Vector2 textSize = _font.MeasureString(_waveMessageText); Vector2 centerPos = new Vector2((spriteBatch.GraphicsDevice.Viewport.Width / 2) - (textSize.X / 2), (spriteBatch.GraphicsDevice.Viewport.Height / 2) - (textSize.Y / 2)); spriteBatch.DrawString(_font, _waveMessageText, centerPos + new Vector2(2, 2), Color.Black); spriteBatch.DrawString(_font, _waveMessageText, centerPos, Color.Gold); }
        else if (_font != null) { spriteBatch.DrawString(_font, $"Oleada: {_waveNumber}", new Vector2(margin, shieldY + 20), Color.White); }
